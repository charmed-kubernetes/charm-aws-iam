...
def _kubectl(*args):
    ...
    command = ['/snap/bin/kubectl',
               '--kubeconfig=' + '/root/.kube/config'] + list(args)
...
def _get_service_ip(service, namespace="kube-system"):
    try:
        output = _kubectl('get', 'service', '--namespace',
                          namespace, service, '--output', 'json')
    ...
    if is_flag_set('charm.aws-iam.deployed-service'):
        ...
        if is_flag_set('endpoint.aws-iam.available'):
            try:
                ...
                hookenv.log(e)
                hookenv.log('Failed to delete AWS_IAM service. Will attempt again next update.')  # noqa
                return
    ...
    if is_flag_set('charm.aws-iam.certificate-written'):
        ...
        if is_flag_set('endpoint.aws-iam.available'):
            try:
                ...
                hookenv.log(e)
                hookenv.log('Failed to delete AWS_IAM secret. Will attempt again next update.')  # noqa
                return
...
def _remove_deployment():
    if is_flag_set('charm.aws-iam.deployment-started'):
        ...
        try:
            ...
            hookenv.log(e)
            hookenv.log('Failed to delete AWS_IAM deployment. Will attempt again next update.')  # noqa
            return
...
def deploy_service():
    ...
    try:
        ...
        hookenv.log(e)
        hookenv.log('Failed to create AWS_IAM service. Will attempt again next update.')  # noqa
        return
...
def waiting_for_certificate_relation():
    ...
    if 'certificates' in goal_state.get('relations', {}):
        hookenv.status_set('waiting',
                           'Waiting for certificate relation to become ready')
    else:
        hookenv.status_set('waiting',
                           'Requires certificate relation established')
...
def write_cert_to_leadership_data():
    ...
    if not leader_get('cluster_id'):
        cluster_id = ''.join(random.choice(string.ascii_letters+string.digits)
                             for i in range(24))
...
def write_cert_secret():
    ...
    try:
        ...
        hookenv.log(e)
        hookenv.log('Failed to create AWS_IAM secret. Will attempt again next update.')  # noqa
        return
...
def apply_webhook_deployment():
    ...
    try:
        ...
        hookenv.log(e)
        hookenv.log('Failed to create AWS_IAM deployment. Will attempt again next update.')  # noqa
        return
    set_flag('charm.aws-iam.deployment-started')


@when('leadership.set.cert', 'endpoint.aws-iam.available',
      'leadership.set.service_ip')
...
def leader_ready():
    try:
        ...
        aws_auth_pods = [pod for pod in out_list['items']
                         if 'aws-iam-authenticator' in pod['metadata']['name']]
        not_running = [pod for pod in aws_auth_pods
                       if (pod['status']['phase'] != 'Running'
                           and pod['status'].get('reason', '') != 'Evicted')
                       or not all([container['ready']
                                   for container in pod['status']['containerStatuses']])] # noqa

        pending = [pod for pod in aws_auth_pods
                   if pod['status']['phase'] == 'Pending']

        if len(pending) > 0:
            hookenv.status_set('maintenance',
                               'Waiting for aws iam pod to start')
